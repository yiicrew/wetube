...
class Selector
{
    ...
    {
        $elements = $this->xpath->evaluate(selectorToXpath($selector));

        return $as_array ? elementsToArray($elements) : $elements;
    }
}
...
function selectElements($selector, $html, $as_array = true)
{
    $dom = new SelectorDOM($html);
...
function elementsToArray($elements)
{
    ...
    for ($i = 0, $length = $elements->length; $i < $length; ++$i) {
        if ($elements->item($i)->nodeType == XML_ELEMENT_NODE) {
            array_push($array, elementToArray($elements->item($i)));
        }
    }
...
function elementToArray($element)
{
    $array = [
        ...
        'children' => elementsToArray($element->childNodes),
    ];
